...
class FrxDonutChart extends Component<FrxDonutChartProps>{
    ...
    componentDidMount = () => {
        ...
        var __data = data.sort(function (a: any, b: any) { return a.key === 'paid' ? 1 : -1 })
        ...
        svg.data(data)
        var total: any = data.reduce((a: any, b: any) => {
            return { value: a.value + b.value }
        }, { value: 0 })['value'];

        // Generate the pie
        var pie = d3.pie().sort(null)
        var _data = pie(__data.map((item: any) => { return (item.value / total) * 100 }))
        ...
        arcs.append("path")
            .attr("fill", function (d: any, i: number) {
                if (__data[i].color) return __data[i].color
                else return __data[i].key === 'paid' ? "#59B35E" : "#F65A1C"
            })
            .each((d: any) => {
                d.outerRadius = __data[d.index].key === 'paid' ? this.outerRadius : this.outerRadius + 11;
            })
            .attr("d", this.arc)
            .attr("d", (d) => {
                return this.arc(d);
        ...
        var circleParent = arcs.append('g').attr('id', function (d: any) {
            return __data[d.index].key
        })
            .style('visibility', function (d: any) {
                return d.data < 1 ? 'hidden' : 'visible';
            })
        ...
        circleParent.append('circle')
            ...
            .attr("fill", function (d: any, i: number) {
                if (__data[i].color) return __data[i].color
                return __data[i].key === 'paid' ? "#59B35E" : "#F65A1C"
            })

        circleParent.append("text")
            .attr("text-anchor", "middle")
            .attr("x", (d: any): number => {
    ...
    render() {
        return (<>
            <svg id="donut" height={250} width={230} style={{ overflow: 'visible'}}>
            </svg>
        </>
        )
    };
